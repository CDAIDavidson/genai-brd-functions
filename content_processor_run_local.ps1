<#
.SYNOPSIS
    Run the content_processor Cloud Function locally with Google Functions Framework.
    • Loads environment variables from .env
    • Listens on http://localhost:8083
    • Optional VS Code debugging on port 5680 when DEBUG=1

.DESCRIPTION
    1. Reads each KEY=VALUE line in .env and exports it.
    2. If $Env:DEBUG is set, starts debugpy so VS Code can attach.
    3. Launches Functions Framework, pointing at src/content_processor.
...
$port      = 8083          # HTTP port for Functions Framework
$debugPort = 5680          # VS Code attach port
